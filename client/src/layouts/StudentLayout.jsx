...
import { LayoutDashboard, HeartHandshake, FileText, User, Award, Clock, LogOut, Menu, Briefcase, QrCode, CreditCard, MessageSquare } from 'lucide-react';
...
const StudentLayout = () => {
    ...
    const navItems = [
        ...
        { label: 'Volunteering', path: '/student/volunteering', icon: HeartHandshake },
        { label: 'My Applications', path: '/student/applications', icon: Briefcase },
        { label: 'Track Hours', path: '/student/hours', icon: Clock },
        { label: 'Internships', path: '/student/internships', icon: Briefcase },
        { label: 'Payments', path: '/student/payments', icon: CreditCard },
        { label: 'Scan QR', path: '/student/scan-qr', icon: QrCode },
        ...
        <div className="flex min-h-screen font-sans bg-slate-50/50">
            ...
            `}>
                ...
                <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="mt-8 text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 px-4">Learning</div>
                    <Link to="/student/certifications" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-white/40 hover:text-slate-900 transition-all duration-300 hover:translate-x-1 group">
                        <Award size={20} className="text-slate-400 group-hover:text-primary-500 transition-colors" />
                        <span className="font-medium">Browse Certifications</span>
                    </Link>
                    <Link to="/student/certificates" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-white/40 hover:text-slate-900 transition-all duration-300 hover:translate-x-1 group">
